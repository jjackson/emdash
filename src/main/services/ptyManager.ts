...
function resolveWindowsPtySpawn(
  ...
  if (ext === '.cmd' || ext === '.bat') {
    ...
    // Pass as a raw string so node-pty doesn't escape our cmd.exe quotes.
    // node-pty's argsToCommandLine escapes " as \" (MSVCRT convention) which
    // breaks cmd.exe. Outer quotes let /s strip them, preserving inner quoting
    // for paths with spaces (e.g. "C:\Users\Jonathan Jackson\...\claude.cmd").
